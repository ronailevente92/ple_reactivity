...
dat_cleaned <- 
  ...
  select(id_esm, age, gender, edu, city, start_date, date_esm, date_cogtask,
         ...
         ple_lose_control = ple_control, ple_thought_control =  ple_strange, ple_unusual_thoughts = ple_thoughts, ple_suspicion,
         ple_mistreat = ple_treatment, ple_perception, ple_paranoia = ple_thougtscontrol, ple_familiarstrange,
...
cfa_ple <-
  ...
  filter(!is.na(date_esm)) %>%
  select(id_esm, date_esm, beep,
         ple_lose_control, ple_thought_control, ple_unusual_thoughts,
         ple_suspicion, ple_mistreat, ple_perception, ple_paranoia, ple_familiarstrange) %>%
  filter_at(vars(ple_lose_control:ple_familiarstrange), all_vars(!is.na(.))) %>%
  distinct() %>%
  ungroup()
...
cfa_ple_syntax <- '
  ...
  level: 1
      ...
      ple_within   =~    ple_lose_control + ple_thought_control + ple_unusual_thoughts + ple_suspicion +
                         ple_mistreat + ple_perception + ple_paranoia + ple_familiarstrange
                         
                         ple_perception	~~	ple_familiarstrange
                         ple_unusual_thoughts	~~	ple_familiarstrange


  level: 2

      ple_between   =~   ple_lose_control + ple_thought_control + ple_unusual_thoughts + ple_suspicion +
                         ple_mistreat + ple_perception + ple_paranoia + ple_familiarstrange
                         
                        ple_lose_control	~~	ple_thought_control
...
dat_ple_lev_1 <-
  ...
  select(-c(ple_lose_control:ple_familiarstrange))
