...
dat_cleaned <- 
  big_data %>% 
  select(id_esm, session_esm, created_esm, age, gender, edu, city, start_date, date_esm, date_diary,
...
cfa_stressor_formative <-
  dat_cleaned %>%
  group_by(id_esm, date_diary) %>%
  filter(!is.na(date_diary)) %>%
  # arrange(id_emo, day, beep) %>% 
  mutate_at(vars(
    angry, irritated, sad,
    unrested, afraid, ple_control, ple_strange, ple_thoughts, ple_suspicion, ple_treatment,
    ple_perception, ple_thougtscontrol, ple_familiarstrange),
    .funs = list(
      mean_threedays = ~ mean(., na.rm = T))
    ) %>% 
  ungroup() %>% 
  distinct() %>%
  select(id_esm, date_diary, stressor_1:stressor_19, stressor_21, contains("mean_threedays")) %>%
  filter_at(vars(stressor_1:ple_familiarstrange_mean_threedays), all_vars(!is.na(.))) %>%
  distinct() %>%
  ungroup()
...
ple =~ 
ple_control_mean_threedays + ple_strange_mean_threedays + ple_thoughts_mean_threedays + ple_suspicion_mean_threedays + ple_treatment_mean_threedays + ple_perception_mean_threedays + ple_thougtscontrol_mean_threedays + ple_familiarstrange_mean_threedays

negaff =~
sad_mean_threedays + afraid_mean_threedays + unrested_mean_threedays + angry_mean_threedays + irritated_mean_threedays
...
model.syntax 93
# 7. formative factors
stressors <~  stressor_1 + stressor_2 + stressor_3 + stressor_4 + stressor_5 +
stressor_6 + stressor_7 + stressor_8 + stressor_9 +  stressor_10 +  stressor_11 +  stressor_12 + stressor_13 + stressor_14 + stressor_15 + stressor_16 +  stressor_17 + stressor_18 + stressor_19 + stressor_21'


fit_formativ_stressor <- lavaan(model = myModel, data = cfa_stressor_formative, std.lv = TRUE, estimator = "ML")
```
...
prin <-
  dat_cleaned %>%
  select(stressor_2:stressor_19, stressor_21, -stressor_8) %>% 
    filter_at(vars(stressor_2:stressor_21), all_vars(!is.na(.)))


(parallel <- psych::fa.parallel(prin, fm = "pa"))

(principal <-
    psych::principal(prin, nfactors = 7, rotate = "oblimin"))
...
cfa_stressor <-
  dat_cleaned %>%
  filter(!is.na(date_diary)) %>%
  select(id_esm, date_diary, stressor_1:stressor_19, stressor_21) %>%
  filter_at(vars(stressor_1:stressor_21), all_vars(!is.na(.))) %>%
  distinct() %>%
  ungroup()
```
...
fit_stressor <- cfa(model = cfa_stressor_syntax, data = cfa_stressor, std.lv = TRUE, estimator = "MLR", cluster = "id_esm")
```
...
cfa_stressor_syntax <- '
  ...
  #    stressor_21
      ...
      stressor_10	~~	stressor_11
      stressor_15	~~	stressor_16
  
'

fit_stressor <- cfa(model = cfa_stressor_syntax, data = cfa_stressor, std.lv = TRUE, estimator = "WLSMV", ordered = T)
```

```{r, warning = T}

fitMeasures(fit_stressor, c("chisq", "df", "pvalue", "cfi", "tli", "rmsea", "srmr", "srmr_within", "srmr_between"))
```

```{r, warning = T}

summary(fit_stressor, fit.measures = TRUE, standardize = TRUE)


modificationindices(fit_stressor) %>%
...
model_dat <-
  dat_cleaned %>%
  select(id_esm, session_esm, created_esm, edu, gender, age, city, start_date, date_diary, day, beep, diary_counter,
