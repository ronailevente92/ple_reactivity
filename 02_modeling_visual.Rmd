...
cucli <- 
  lmer(
    event_unpleasantness ~
      age + gender + beep + day +
      stressor_sum_centered_int * nback_dprime_grand_centered +
...
ggsave("esm.jpeg", device = "jpeg", width = 20, height = 12, units = "cm", )

```

```{r, fig.height=5, fig.width=5}
model_dat %>% 
  pivot_longer(names_to = "affect", cols = c(posaff_cfa, negaff_cfa, ), values_to = "score" ) %>% 
  group_by(day, id_esm, affect) %>% 
  summarise(score = mean(score, na.rm = T)) %>% 
  ungroup() %>% 
  mutate(has_data = ifelse(!is.na(score),
                           1,
                           0)) %>% 
  group_by(id_esm) %>%
  mutate(n_data = sum(as.numeric(has_data))) %>% 
  select(-has_data) %>% 
  filter(n_data > 49 & id_esm > 150 & id_esm < 180) %>% 
  ggplot(aes(day, score)) + 
  geom_point(aes(color = affect), alpha = .7, size = 2) + 
  geom_line(aes(group = id_esm), alpha = .4) + 
#  geom_smooth() + 
  lims(x = c(1, 28)) + 
  facet_wrap(~id_esm+affect, ncol = 2)  +
  theme_bw() +
  theme(
...
ggsave("esm_aff.jpeg", device = "jpeg", width = 20, height = 20, units = "cm", )
...
corr_cog <- 
  model_dat %>% 
  dplyr::select(id_esm, stressor_sum_centered_int, event_unpleasantness_centered, negaff_cfa_centered) %>% 
 # mutate(stressor_sum = stressor_sum * -1) %>% 
  distinct()

shapiro.test(corr_cog$stressor_sum_centered_int)
shapiro.test(corr_cog$event_unpleasantness_centered)
shapiro.test(corr_cog$negaff_cfa_centered)
# normality not assumed

cor.test(corr_cog$stressor_sum_centered_int, corr_cog$negaff_cfa_centered, method = "spearman")
# cor.test(corr_cog$mss_dis_score, corr_cog$stressor_sum_centered_int, method = "spearman")
...
ggplot(corr_cog, aes(event_unpleasantness_centered, negaff_cfa_centered)) +
...
m4 <- 
  lmer(
    ple_cfa ~ 
      ...
      (event_unpleasantness_centered | id_esm), data = model_dat %>% filter(!is.na(ple_cfa_centered_lag)))


tab_model(m2, m4, m3, show.aicc = T, show.std = T)
plot_model(m4, type = "int")
...
# 
# vars <- c("ple_cfa", "gonogo_dprime", "nback_dprime", "event_unpleasantness")
