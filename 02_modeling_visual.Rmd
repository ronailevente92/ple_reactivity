...
corr_cog <- 
  model_dat %>% 
  dplyr::select(id_esm, stressor_sum, event_unpleasantness_centered, mss_dis_score) %>% 
  mutate(stressor_sum = stressor_sum * -1) %>% 
  distinct()

shapiro.test(corr_cog$stressor_sum)
shapiro.test(corr_cog$event_unpleasantness_centered)
shapiro.test(corr_cog$mss_dis_score)
# normality not assumed

cor.test(corr_cog$mss_dis_score, corr_cog$stressor_sum, method = "spearman")
...
ggplot(corr_cog, aes(mss_dis_score, stressor_sum)) +
